...
router.post('/draw', function (req, res, next) {
  if (req.body._id) {
    dbTool.findOne("token", { token: req.get("Authorization") }, function (results) {
      if (typeof (results) == "object") {
        if (!results.drawType) {
          ...
          dbTool.updateOne(collectionName, whereStr, updateStr, function (result) {
            ...
            dbTool.updateOne("token", { token: req.get("Authorization") }, updateStr2, function (results) {
          ...
          res.json(jsonTool.toObj({ type: "havedrawed" }))
        }
      } else {
        res.json(jsonTool.justCodeInt(false, "未找到该条数据"))
      }
    ...
    res.json(jsonTool.justCodeInt(false, "获取参数失败"))
  }
});
